...
const Navbar = () => {
  const resNavItem = ["Home", "About", "Pricing", "Blog"];
      ...
      <nav className="w-full shadow-2xl shadow-secondary/30 relative">
        <div className="w-full flex justify-between items-center bg-background border border-neutral-300 ring-1 ring-secondary/30 ring-inset p-2.5 rounded-2xl">
          ...
          <div className="hidden md:flex justify-center items-center md:gap-1 lg:gap-3">
            <NavBtn text={"Home"} ref={"home"} />
            <NavBtn text={"About Us"} ref={"about"} />
            <NavBtn text={"Pricing"} ref={"pricing"} />
            <NavBtn text={"Blog"} ref={"blog"} />
          </div>
          ...
          <button onClick={handleNavToggle} className="block md:hidden pr-1">
            <Sidebar2
              size={24}
              className={`text-primary ${navToggle ? "hidden" : "block"}`}
            />
            <Sidebar2
              size={24}
              className={`text-primary rotate-180 ${navToggle ? "block" : "hidden"}`}
            />
          </button>
        </div>

        <div
          className={`w-full ${navToggle ? "block" : "hidden"} absolute top-16 left-1/2 -translate-x-1/2 bg-background border border-neutral-300 ring-1 ring-secondary/30 ring-inset p-1 rounded-2xl`}
        >
          {resNavItem.map((i) => (
            <button
